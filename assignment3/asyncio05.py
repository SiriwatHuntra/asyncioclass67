...
async def fetch_hacker_news():
    async with aiohttp.ClientSession() as session:
        ...
        url2 = f'http://hacker-news.firebaseio.com/v0/item/24659683.json?print=pretty'
        task2 = asyncio.create_task(get_api(session, url2))

        news1 = await task1 #wait till task 1 complete
        news2 = await task2 #await till task 2 complete
